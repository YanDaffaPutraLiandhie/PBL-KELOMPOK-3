...
const ConfigurationPage = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[80vh] px-2 mt-12 md:mt-16">
      <div className="w-full max-w-3xl bg-[var(--card-bg)] rounded-2xl shadow-2xl p-8 border border-[var(--border)]">
        {loading && (
          <div className="text-center text-sm text-[var(--text-muted)] mb-4">Memuat konfigurasi dari server...</div>
        )}
        {/* Tombol Kembali */}
        <div className="mb-6">
          <Link href="/" className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-[var(--bg-600)] border border-[var(--border)] text-xs text-[var(--primary)] font-mono shadow hover:scale-105 transition-all">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 mt-2">
          ...
          <div className="flex flex-col md:flex-row gap-4 mt-2">
            <button type="submit" className="w-full md:w-1/2 py-2 rounded-lg bg-gradient-to-r from-cyan-400 to-green-400 text-white font-bold text-lg shadow hover:scale-[1.03] transition-all">Simpan Konfigurasi</button>
            <button type="button" onClick={handleReset} className="w-full md:w-1/2 py-2 rounded-lg bg-gradient-to-r from-gray-600 to-gray-800 text-white font-bold text-lg shadow hover:scale-[1.03] transition-all">Reset ke Default</button>
          </div>
        </form>
      </div>
    </div>
